...
CheckShellParameter(){
  ...
  ## 使用者說明
  printUsage(){
    echo " Purpose: Print shoe"
    echo " Usage: bash `basename $0` [性別<m|f>] [鞋碼種類<us|gb|eu|jp|tw>] [尺寸]"
    echo "  exp1: bash `basename $0` m us 10.5"
    echo "  exp2: bash `basename $0` f eu 40"
    echo "  exp3: bash `basename $0` m gb 10.5"
...
#  P5=$5;
  ...
  ## 檢查參數數量 ##
  if [ $ParameterNums -ne 3 ]; then
  	echo "ParameterNums=[$ParameterNums] 參數數量錯誤" | tee -a ${LOGFILE};
  	printUsage 1101;
  fi
  
  ###檢查[性別]參數是否錯誤 ##
  if [[ $P1 != "m" ]] && [[ $P1 != "f" ]]; then ##檢查數值合理性
    echo "性別=[$P1] 請輸入正確的性別" | tee -a ${LOGFILE};
    printUsage 1102;
  fi
  
  ###檢查[鞋碼種類]參數是否錯誤 ##
  if [[ $P2 != "us" ]] && [[ $P2 != "gb" ]] && [[ $P2 != "eu" ]] && [[ $P2 != "jp" ]] && [[ $P2 != "tw" ]]; then ##檢查數值合理性
    echo "鞋碼種類=[$P2] 請輸入正確的鞋碼種類" | tee -a ${LOGFILE};
    printUsage 1102;
  fi
  ...
  ###檢查[尺寸]參數是否錯誤 ##
  if [[ ! $P3 =~ $NUregex ]]; then
    echo "尺寸=[$P3] 請輸入正確的體重" | tee -a ${LOGFILE};
    printUsage 1102;
  elif [ `echo "$P3 < 1" | bc` -eq 1 ] || [ `echo "100 < $P3" | bc` -eq 1 ]; then
    echo "尺寸=[$P3] 請輸入正確的體重" | tee -a ${LOGFILE};
    printUsage 1102;
  fi
...
SH1003Process $AllParameterNums;
